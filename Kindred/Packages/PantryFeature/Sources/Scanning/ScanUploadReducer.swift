...
// Note: AnalyzeScanMutation will be generated by Apollo codegen from AnalyzeScan.graphql
...
public struct ScanUploadReducer {
    ...
    public enum Action {
        ...
        case analysisCompleted([DetectedItem])
        ...
        public enum Delegate {
            ...
            case analysisCompleted([DetectedItem], ScanJob)
        }
    }
    ...
    public var body: some ReducerOf<Self> {
        Reduce { state, action in
            ...
            case let .uploadCompleted(job):
                ...
                return .run { send in
                    ...
                    do {
                        ...
                        let mutation = AnalyzeScanMutation(jobId: jobId, userId: userId)
            ...
            case let .analysisCompleted(items):
                state.uploadState = .completed
                guard let scanJob = state.scanJob else { return .none }
                return .send(.delegate(.analysisCompleted(items, scanJob)))

            case let .analysisFailed(errorMessage):
                state.uploadState = .failed
                state.error = String(localized: "scan.failure.retry_or_manual", defaultValue: "We couldn't identify items. Retry or add items manually?", bundle: .main)
                return .none
